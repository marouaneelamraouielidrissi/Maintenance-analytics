...
function envoyerRappelPDR() {
  ...
  function buildHtml(posteLabel, dest, pendingList) {
    ...
    for (var k = 0; k < pendingList.length; k++) {
      ...
      rows += '<tr style="background:' + bg + ';border-bottom:1px solid #e5e7eb;">'
    ...
    return '<!DOCTYPE html><html><body style="font-family:Arial,sans-serif;color:#374151;background:#f9fafb;margin:0;padding:0;">'
         ...
         + '<p style="margin:4px 0 0;color:#93c5fd;font-size:13px;">' + posteLabel + ' — ' + pendingList.length + ' PDR</p>'
         + '</div>'
         ...
         + '<th style="padding:10px 12px;text-align:left;">N&deg; OT</th>'
         + '<th style="padding:10px 12px;text-align:left;">Objet technique</th>'
         + '<th style="padding:10px 12px;text-align:left;">Description</th>'
         + '<th style="padding:10px 12px;text-align:left;">PDR demandee</th>'
         + '<th style="padding:10px 12px;text-align:left;">Poste</th>'
         + '</tr></thead>'
         + '<tbody>' + rows + '</tbody>'
         + '</table>'
         + '</div>'
  ...
  // 421-MEC + 421-CHAU (standards) → un seul mail à Appro mécanique
  var pendingMec = getPending('421-MEC').concat(getPending('421-CHAU'));
  sendEmail(pendingMec, 'Appro mécanique', CC_APPRO_MEC, '421-MEC + 421-CHAU');
  ...
  // 423-ELEC + 423-REG → un seul mail à Appro électrique
  var pendingElec = getPending('423-ELEC').concat(getPending('423-REG'));
  sendEmail(pendingElec, 'Appro électrique', CC_BASE, '423-ELEC + 423-REG');
}
