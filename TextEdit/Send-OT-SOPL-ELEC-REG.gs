...
// ═══════════════════════════════════════════════════════════════
function SEND_OT_SOPL_ELEC_REG() {
  const feuille   = SpreadsheetApp.getActiveSpreadsheet();
  const feuilleID = feuille.getId();
  const token     = ScriptApp.getOAuthToken();

  const feuilles = [
    { nom: "OT à envoyer électrique",  nomFichier: "OT_PLANIFIES_ELECTRIQUE_ " },
    { nom: "OT à envoyer régulation",  nomFichier: "OT_PLANIFIES_REGULATION_ " }
  ];
    ...
    row.forEach(function(cell, colIndex) {
      if (rowIndex === 0) {
        ...
        if (colIndex === 3 || colIndex === 4) {
          ...
          else if (val >= 50) style += "background-color:#ffeb9c;color:#9c6500;font-weight:bold;";
  ...
  const sujet = "Liste des OT Planifié Electrique & Régulation - " +
...
function tester_OT_SOPL_ELEC_REG() {
  const feuille   = SpreadsheetApp.getActiveSpreadsheet();
  const feuilleID = feuille.getId();
  const token     = ScriptApp.getOAuthToken();
  ...
  sendEmailEWS(
    OCP_EMAIL_EWS, null,
    '[TEST] OT Planifiés Électrique & Régulation',
