...
// ═══════════════════════════════════════════════════════════════
function envoyer_AVIS_MEC_INST() {
  const feuille   = SpreadsheetApp.getActiveSpreadsheet();
  const feuilleID = feuille.getId();
  const token     = ScriptApp.getOAuthToken();

  const feuilles = [
    { nom: "Avis à envoyer mécanique",    nomFichier: "AVIS_OUVERTS_MECANIQUE - " },
    { nom: "Avis à envoyer installation", nomFichier: "AVIS_OUVERTS_INSTALLATION - " }
  ];
  ...
  const cc  = "m.mamouni@ocpgroup.ma, elkhyari@ocpgroup.ma, ibtissame.elkhloufi@ocpgroup.ma, m.mabdoui@ocpgroup.ma, lazrag@ocpgroup.ma, m.elamraoui@ocpgroup.ma";
  const sujet = "Liste des Avis ouverts (Mécanique & Installation) - " +
...
function tester_AVIS_MEC_INST() {
  ...
  sendEmailEWS(
    OCP_EMAIL_EWS, null,
    '[TEST] Avis ouverts Mécanique & Installation',
